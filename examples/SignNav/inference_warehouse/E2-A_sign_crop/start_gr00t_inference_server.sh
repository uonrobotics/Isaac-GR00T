...
if [[ -z "$MODEL_PATH" || ! -f "$MODEL_PATH/config.json" ]]; then
    echo "[E2-A] could not resolve a usable checkpoint under: $RUN_ROOT" >&2
    echo "[E2-A] note: checkpoints without processor_config.json are skipped." >&2
    exit 1
fi
